...
sap.ui.define(["sap/ui/core/format/DateFormat", "jquery.sap.global"], function (DateFormat, jQuery) {
    "use strict";
     ...
     */
    var fnCalculateAge = function fnCalculateAge(oBirthday) {
    ...
    var fnCalculateSize = function fnCalculateSize(aProjects) {
        var fnSumTagLength = function fnSumTagLength(s, tag) {
            return s + tag.name.length;
        };
        var fnComputeRaw = function fnComputeRaw(oProject) {
            return oProject.description.length + oProject.technologies.reduce(fnSumTagLength, 0) * nTagFactor + oProject.complexity.length * nTagFactor;
        };
        aProjects.forEach(function (oProject) {
            return oProject.size = fnComputeRaw(oProject);
        });
        var nMax = Math.max.apply(Math, _toConsumableArray(aProjects.map(function (p) {
            return p.size;
        })));
        aProjects.forEach(function (oProject) {
            return oProject.size = Math.max(3, Math.ceil(oProject.size * 8 / nMax));
        });
    };

    var fnCalculateSizeForAll = function fnCalculateSizeForAll(mProjects) {
        var aDeep = jQuery.map(mProjects, function (t) {
            return t;
        });
        fnCalculateSize(aDeep.reduce(function (a, b) {
            return a.concat(b);
        }, []));
    };
    ...
    return function (i18n) {
        var oBirthday = new Date(1993, 8, 7),
            ...
            fnReplacei18n = function fnReplacei18n(oObject) {
            var sKey = void 0,
                oResult = void 0;
            ...
            if ((typeof oObject === "undefined" ? "undefined" : _typeof(oObject)) === "object") {
                if (oObject.hasOwnProperty("_text")) {
                    return jQuery.sap.formatMessage(fnReplacei18n(oObject._text), fnReplacei18n(oObject._params));
                } else {
                    oResult = {};
                    for (sKey in oObject) {
